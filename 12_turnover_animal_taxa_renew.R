...
#ants_mds = metaMDS(ants_pa, distance = "jaccard", na.rm = TRUE)
...
#ants_mlt = beta.multi(ants_pa, index.family = "jaccard")
#ants_mlt_abnd = beta.multi.abund(ants, index.family = "bray")

#str(ants_mlt)
#ants_mlt$beta.JTU

#tst = betapart.core(ants_pa)
#ants_mlt_sites = beta.sample(tst, 
#                             index.family = "jaccard", 
#                             sites = 2, samples = 59)
#tst_tst = beta.multi(tst, index.family = "jaccard")

#library(mefa)
ants_tst_pairs = beta.pair(ants_pa, index.family = "jaccard")
str(ants_tst_pairs)
str(ants_tst_pairs[[1]])

ants_tst_pairs_cmd = cmdscale(ants_tst_pairs$beta.jac)
ants_tst_pairs_cln = ifelse(ants_tst_pairs[[1]] == "NaN", NA, ants_tst_pairs[[1]])

ants_tst_pairs[[1]] = ifelse(ants_tst_pairs[[1]] == "NaN", 0, 
                                 ants_tst_pairs[[1]])

ants_mds_jtu = metaMDS(na.omit(ants_tst_pairs[[1]]), 
                       distance = "euclidean", na.rm = TRUE)

ants_mds_jtu_sites = scores(ants_mds_jtu, choices = c(1,2), display = "sites")
colnames(ants_mds_jtu_sites) = c("ants_NMDS1", "ants_NMDS2")
...
bees_mlt = beta.multi(bees_pa, index.family = "jaccard")

bees_mlt_sites = beta.sample(bees_pa, 
                             index.family = "jaccard", 
                             sites = 59, samples = 60)
...
beetles_mds_sites$beetles_NMDS1 = as.numeric(beetles_mds_sites$beetles_NMDS1)
beetles_mds_sites$beetles_NMDS2 = as.numeric(beetles_mds_sites$beetles_NMDS2)
